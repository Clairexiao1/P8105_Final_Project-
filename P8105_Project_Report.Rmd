...
michelin_2020 <-
  ...
  rename(star = stars) %>%
  mutate(year = 2020) %>%
  relocate(name,year,city,latitude,longitude)

michelin_map <-
  michelin_df %>%
  select(name, year, city, latitude, longitude,star) %>%
  rbind(michelin_2020) %>%
  mutate(
